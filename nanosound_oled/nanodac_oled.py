...
class Screen:
    ...
    def __init__(self, dev):
        ...
        self.fonts  = {
          ...
          'big':     self.make_font('DejaVuSansMono-Bold.ttf', 16),
        }
    ...
    def logo(self, lanip="", wlanip=""):
        ...
        with canvas(device) as draw:
            ...
            draw.text((3, 40), 'NanoSound v1.5.2', font=self.fonts['small'], fill='white')
    ...
    def draw(self, data):
        ...
        with canvas(device) as draw:
            ...
            if self.artistScroll == None or txt != self.artistScroll.text:
                self.artistScroll = self.Scroll(self.device.width, self.device.height, txt, self.fonts['medium'], 11, 6)
            if self.titleScroll == None or data['title'] != self.titleScroll.text:
                self.titleScroll  = self.Scroll(self.device.width, self.device.height, data['title'], self.fonts['big'], 20, 12)
            ...
            if 'samplerate' in data and 'bitdepth' in data:
                if str(data['samplerate']) == 'Volspotconnect2':
                    ...
                    txt += data['bitdepth'].replace(" bit", '') + '/' \
                         + data['samplerate'].replace(" KHz", '')
